...
class ntrnx_mysqli_internal_check_update {
	...
	public function check_update($class_local = NULL) {
		...
		define('_MSG_UPDATE_YOU_HAVE_THE_LATEST_VERSION', 	'no updated needed, you have the latest version %v (%d)</br/>');
		define('_MSG_UPDATE_THERE_IS_A_UPDATE_AVAILABE', 	'there is a update available for %s to version %v (%d)');
		define('_MSG_UPDATE_DIRECT_DOWNLOAD_FILE', 			'<br/>direct download: <a href="%u">%l</a></br/>');

		define('_MSG_ERROR_UNKNOWN_VERSION_OF_LOCAL_FILE', 	'unknown version of local file</br/>');
		define('_MSG_ERROR_CANT_COMPARE_VERSION', 			'cant compare version of local and remote file</br/>');
		define('_MSG_ERROR_COULD_GET_UPDATE_INFORMATIONS', 	'could get update information for %s</br/>');
		...
		if (($remoteVersion = @file_get_contents($latest_version))
			...
			&& ($remoteFile = @file_get_contents($latest_file))) {
				...
				if (version_compare($localVersion, $remoteVersion, '<')) {
					...
					$out_str = str_replace("%s", $class_local::_CLASS_NAME, _MSG_UPDATE_THERE_IS_A_UPDATE_AVAILABE);
					$out_str = str_replace("%v", $remoteVersion, $out_str);
					$out_str = str_replace("%d", $remoteDate, $out_str);
					print $out_str;
					
					/* generate direct download link */
					$out_file = str_replace("%u", $remoteFile, _MSG_UPDATE_DIRECT_DOWNLOAD_FILE);
				...
				/* check version if locale version greater than remote version */
					...
					print _MSG_ERROR_UNKNOWN_VERSION_OF_LOCAL_FILE;
					
				/* check version if locale version equal to remote version */
				} else if (version_compare($localVersion, $remoteVersion, '=')) {
					
					$out_file = str_replace("%v", $localVersion, _MSG_UPDATE_YOU_HAVE_THE_LATEST_VERSION);
				...
				/* if version compare not possible */
					...
					print _MSG_ERROR_CANT_COMPARE_VERSION;
		...
		/* if no update informations found */
			...
			print str_replace("%s", $class_local::_CLASS_NAME, _MSG_ERROR_COULD_GET_UPDATE_INFORMATIONS);
